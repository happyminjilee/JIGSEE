                ...
                <div className={styled.content}>
                    {list.map((lst, index) => (
                        <div className={styled.card}>
                            {lst.status === "PUBLISH" && <div><RadioButtonUncheckedIcon color="primary"/> 발행</div>}
                            {lst.status === "PROGRESS" && <div><BuildCircleIcon color="primary"/> 진행 중</div>}
                            {lst.status === "FINISH" && <div><CheckCircleIcon color="primary"/> 완료</div>}
                            <div>
                                {lst.model}
                            </div>
                            {lst.updatedAt === "" ?
